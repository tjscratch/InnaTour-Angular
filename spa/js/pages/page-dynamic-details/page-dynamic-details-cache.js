innaAppControllers
    .controller('PageHotelDetailsCache', [
        'EventManager',
        '$window',
        '$http',
        '$scope',
        ...
        'gtm',
        'dataService',
        function (EventManager, $window, $http, $scope, $rootScope, serviceCache, $timeout, aviaHelper, Urls, Events, $location, DynamicPackagesDataProvider, $routeParams, $route, DynamicFormSubmitListener, $q, $anchorScroll, Balloon, $filter,
            ...
            gtm.GtmTrack(
                {
                    'PageType': 'PackagesDetailsIsLanding'
                },
                {
            ...
            var StartVoyageDateGoBack, EndVoyageDateGoBack, searchParams;
            
            $http({
                url   : "http://localhost:3000/offers/" + routParam.departureSlug + "/" + routParam.SearchDate,
                method: 'GET'
            }).then(
                function (res) {
                    var data = res.data;
                    ...
                    console.log($filter('date')(data.StartVoyageDate, 'yyyy-MM-dd'));
                    
                    StartVoyageDateGoBack = $filter('date')(data.StartVoyageDate, 'yyyy-MM-dd');
                    EndVoyageDateGoBack = $filter('date')(data.EndVoyageDate, 'yyyy-MM-dd');
    
                    var searchParams = angular.extend(routParam, {
                        StartVoyageDate: $filter('date')(data.StartVoyageDate, 'yyyy-MM-dd'),
                        EndVoyageDate  : $filter('date')(data.EndVoyageDate, 'yyyy-MM-dd')
                    });
    
                    if (routParam.Children) {
                        searchParams.ChildrenAges = null;
                    }
