			...
			var FilterPrice = ClassFilter.extend({
					...
					this.on({
						change        : function (data) {
							if (data['FilterData.Value'] && data['setValue']) {
								...
								var priceValue = parseInt(data['FilterData.Value'].replace(/\s+/g, ''), 10);
						...
						setFilterPrice: function (data) {
							...
							if (val >= minPrice) {
								this.set({
									...
									'FilterData.Value': $filter('price')(val)
								});
							}
						},
				...
				mergeData: function () {
					...
					if (this.SaveData.length) {
						...
						this.set('FilterData.Value', $filter('price')(that.SaveData[0]));
					}
				},
				...
				IndicatorFiltersItemRemove: function (data) {
					...
					this.set({
						'value.val'       : [],
						'FilterData.Value': $filter('price')(that.get('FilterData.Max'))
					});
					this.spliceSaveData();
					this._slider.slider('value', this.get('FilterData.Max'));
